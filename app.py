...
"""
Application Streamlit pour la simulation financière SAS France & SARL Sénégal
"""
...
# Initialisation des paramètres dans la session state
...
# Fonction pour créer un widget de paramètre avec une valeur par défaut
def param_widget(label, key, min_value=None, max_value=None, step=None, format=None, help=None):
    ...
    # Déterminer le type de widget en fonction du type de valeur
    if isinstance(value, bool):
        ...
        if key.startswith('taux_'):
            # Pour les taux, afficher en pourcentage (conversion de 0-1 à 0-100)
            value_percent = value * 100  # Convert from 0-1 to 0-100
            ...
            result_percent = st.sidebar.slider(label,
                                     ...
                                     value=value_percent,
                                     step=step_percent,
                                     format="%g%%", # Use %g to avoid trailing zeros
                                     key=key,
                                     help=help)

            # Convert back to 0-1 format for internal use
...
with st.sidebar.expander("Paramètres du scénario 2", expanded=False):
    ...
    def param_widget_scenario2(label, key, min_value=None, max_value=None, step=None, format=None, help=None):
        ...
        if isinstance(value, bool):
            ...
            if key.startswith('taux_'):
                # Pour les taux, afficher en pourcentage
    ...
    # Paramètres les plus importants pour le scénario 2
    ...
    # Bouton pour copier les paramètres du scénario 1
...
if st.button("Exécuter la simulation"):
    with st.spinner("Calcul des résultats en cours..."):
        ...
        with tab1:
            ...
            # Bouton de téléchargement des résultats annuels
            ...
            # Bouton de téléchargement des résultats trimestriels
        ...
        with tab2:
            ...
            with viz_tab3:
                ...
                # Choix entre valeurs absolues et pourcentages
                view_type = st.radio(
